...
// Fetch questions and options from the Python table
$sql = "SELECT * FROM Python";
$result = mysqli_query($conn, $sql);

// Create an array to store MCQ questions and options
$mcqData = array();
...
<body class="scrollbar">
            ...
            <div class="row">
              <div class="col">
                <form action="process_answers.php" method="post">
                  ...
                  <?php
                  foreach ($mcqData as $row) {
                    echo "<p>{$row['QuestionText']}</p>";
                    echo "<ul>";
                    echo "<li><input type='radio' name='q{$row['MCQID']}' value='A' required> {$row['OptionA']}</li>";
                    echo "<li><input type='radio' name='q{$row['MCQID']}' value='B' required> {$row['OptionB']}</li>";
                    echo "<li><input type='radio' name='q{$row['MCQID']}' value='C' required> {$row['OptionC']}</li>";
                    echo "<li><input type='radio' name='q{$row['MCQID']}' value='D' required> {$row['OptionD']}</li>";
                    echo "</ul>";
                    echo "<br>"; // Add some spacing
                  }
                  ?>
  ...
  <script>
    document.getElementById('mcqForm').addEventListener('submit', function (event) {
      // Loop through radio inputs and check if at least one is selected for each question
      var radioGroups = document.querySelectorAll('[name^="q"]');
      var unansweredQuestions = [];
      radioGroups.forEach(function (group) {
        if (!group.querySelector(':checked')) {
          unansweredQuestions.push(group.getAttribute('name'));
        }
      });
  ...
  In this example, a JavaScript function is added that listens for the form's submission event. It checks each radio
  group to see if any option is selected. If a group doesn't have any selected option, it adds the group's name
  (question ID) to the unansweredQuestions array. If there are unanswered questions, the form submission is prevented,
  and an alert message is shown to the user listing the unanswered questions.
